import { Activity, ArrowUpRight, BrainCircuit, ChevronDown, Code2, ExternalLink, Grid2X2, Radar, ShieldCheck, Sparkles, X } from 'lucide-react';
...
const groups = [
  ...
  },
  {
    ...
    items: [
      ...
      ['/portfolio/golden-essence', 'Golden Essence', 'Mobile service website'],
  ...
  return (
    <div className="relative border-t border-white/8 bg-[#05070a]/95">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-4 py-2.5 sm:px-5">
        <button type="button" onClick={() => setOpen((value) => !value)} className="inline-flex min-h-9 items-center gap-2 rounded-full border border-white/10 bg-white/[0.035] px-3 font-mono text-[9px] font-bold uppercase tracking-[0.13em] text-slate-300 transition hover:border-cyan-300/30 hover:text-cyan-200" aria-expanded={open}>
          ...
          <div className="mx-auto max-h-[calc(100svh-118px)] max-w-7xl overflow-y-auto px-4 py-5 sm:px-5 md:py-6">
            <div className="relative overflow-hidden rounded-3xl border border-fuchsia-300/20 bg-[radial-gradient(circle_at_80%_10%,rgba(217,70,239,.16),transparent_30%),radial-gradient(circle_at_10%_90%,rgba(34,211,238,.13),transparent_28%),rgba(255,255,255,.025)] p-5 md:p-6">
              ...
              <div className="grid gap-5 lg:grid-cols-[1.05fr_.95fr] lg:items-center">
                <div>
                  <div className="inline-flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.16em] text-fuchsia-300"><Radar size={13}/> Featured rabbit hole</div>
                  <h2 className="mt-3 max-w-2xl font-display text-3xl font-bold leading-tight md:text-4xl">Misfit Trader is what happens when the same guarded-agent thinking touches markets.</h2>
                  <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">Live market emotion, real candle data, plain-English pattern intelligence, autonomous paper portfolios and prediction-market signals. Real-money authority stays gated.</p>
                  <Link to="/signal" onClick={() => setOpen(false)} className="mt-5 inline-flex items-center gap-2 rounded-2xl bg-fuchsia-300 px-4 py-3 font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-black">Open Misfit Trader <Activity size={14}/></Link>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <div className="rounded-2xl border border-cyan-300/15 bg-cyan-300/[0.04] p-4"><Code2 size={16} className="text-cyan-300"/><div className="mt-4 text-xs font-semibold">Context</div><div className="mt-1 text-[10px] leading-4 text-slate-600">Read the field before acting.</div></div>
                  <div className="rounded-2xl border border-fuchsia-300/15 bg-fuchsia-300/[0.04] p-4"><ShieldCheck size={16} className="text-fuchsia-300"/><div className="mt-4 text-xs font-semibold">Guardrails</div><div className="mt-1 text-[10px] leading-4 text-slate-600">Paper first. Authority bounded.</div></div>
                  <div className="rounded-2xl border border-amber-300/15 bg-amber-300/[0.04] p-4"><BrainCircuit size={16} className="text-amber-300"/><div className="mt-4 text-xs font-semibold">Reasoning</div><div className="mt-1 text-[10px] leading-4 text-slate-600">Signal is evidence, not certainty.</div></div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {groups.map((group) => (
                <section key={group.title} className="rounded-3xl border border-white/8 bg-white/[0.018] p-4">
                  <div className="mb-3 flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.15em] text-cyan-300"><Sparkles size={11}/>{group.title}</div>
                  <div className="space-y-2">{group.items.map(([href,label,note]) => <Destination key={`${href}-${label}`} href={href} label={label} note={note} onOpen={() => setOpen(false)}/>)}</div>
                </section>
              ))}
            </div>

            <div className="mt-4 flex items-center gap-3 rounded-2xl border border-white/8 bg-black/40 p-4">
              <img src="/neon-skull-rose.png" alt="" className="h-12 w-12 rounded-xl object-cover opacity-90"/>
              <div><div className="font-mono text-[9px] uppercase tracking-[0.14em] text-slate-500">Misfit Mediahouse public surface</div><div className="mt-1 text-sm text-slate-300">Built in public. Kernel private. And yes — this whole operating estate was built from a phone.</div></div>
            </div>
          </div>
        </div>
      )}
