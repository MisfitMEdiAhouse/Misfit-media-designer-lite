...
export default function GTAGrowthOS(){
  ...
  useEffect(()=>{
    ...
    if(q.get('checkout')==='success')setNotice('PAYMENT RECEIVED · Misfit Cloud is provisioning your managed GTA Growth OS onboarding now.');
    if(q.get('checkout')==='cancelled')setNotice('Checkout cancelled. Your audit and free-pilot option stay here; no subscription was created.');
  },[]);
  ...
  const runAudit=async(e)=>{
    e.preventDefault();setBusy('audit');setNotice('');setAudit(null);setPilotSubmitted(false);
    try{
  ...
  const applyForPilot=async()=>{
    setBusy('pilot');setNotice('');
    try{
      await catalog({
        operation:'submit_gta_request',
        request_type:'server_service',
        name:form.community_name.trim(),
        email:form.email.trim().toLowerCase(),
        message:`FOUNDING GTA GROWTH PILOT · Goal: ${form.goal} · Community URL: ${form.website.trim()} · Audit: ${audit?.score??'n/a'}/100 grade ${audit?.grade??'n/a'}. Requesting a free pilot to prove measurable growth before a paid managed upgrade.`,
        source:'gta_growth_free_pilot'
      });
      setPilotSubmitted(true);
      setNotice('FOUNDING PILOT REQUEST RECEIVED · No card charged. The goal is to prove measurable growth before you pay for managed scale.');
    }catch(error){setNotice(`PILOT — ${error.message}`)}finally{setBusy('')}
  };
  ...
  return <section className="mt-5 overflow-hidden rounded-3xl border border-cyan-300/20 bg-[radial-gradient(circle_at_15%_0%,rgba(34,211,238,.12),transparent_32%),radial-gradient(circle_at_90%_20%,rgba(217,70,239,.10),transparent_30%),rgba(255,255,255,.02)] p-4 md:p-6">
    <div className="grid gap-5 lg:grid-cols-[1.05fr_.95fr]">
      <div>
        <div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/[.06] px-3 py-1.5 font-mono text-[9px] font-bold tracking-[.14em] text-cyan-200"><TrendingUp size={14}/> MISFIT GTA GROWTH OS · FREE TO PROVE</div>
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">Run the public-site audit free. If the fit is real, apply for a free Founding Pilot so Misfit can prove measurable growth before you pay. Managed Growth remains $299/month for communities ready to put Misfit on the growth loop now.</p>
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">{['FREE AUDIT','FREE PILOT','GROWTH ANALYTICS','MANAGED $299'].map(x=><div key={x} className="rounded-xl border border-white/10 bg-black/35 p-3 font-mono text-[9px] text-slate-400">{x}</div>)}</div>
        <div className="mt-4 rounded-2xl border border-fuchsia-300/15 bg-fuchsia-300/[.035] p-4"><div className="font-mono text-[10px] text-fuchsia-200">FREE TO PROVE · $299/MO TO SCALE</div><div className="mt-1 text-xl font-bold">Founding Pilot → Managed Growth</div><p className="mt-2 text-xs leading-5 text-slate-500">The pilot does not require a card and does not create a subscription. Managed Growth is customer-authorized through Stripe. No autonomous charges, no in-game item sales, no game-server hosting and no crypto/token economy.</p></div>
      </div>
    ...
    {audit&&<div className="mt-5 rounded-2xl border border-emerald-300/20 bg-emerald-300/[.035] p-4 md:p-5">
      ...
      <div className="mt-4 rounded-xl border border-cyan-300/20 bg-cyan-300/[.04] p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="font-mono text-[9px] text-cyan-300">FOUNDING PILOT · $0</div><div className="mt-1 text-lg font-bold">Make Misfit prove the growth before you pay.</div><p className="mt-1 text-xs text-slate-500">Apply with this audit attached. No card, no subscription, no automatic charge.</p></div><button type="button" onClick={applyForPilot} disabled={Boolean(busy)||pilotSubmitted} className="min-h-12 shrink-0 rounded-xl bg-cyan-300 px-5 font-mono text-[10px] font-black text-black disabled:opacity-50">{pilotSubmitted?'PILOT REQUESTED ✓':busy==='pilot'?'SUBMITTING…':'APPLY FREE →'}</button></div>
        <div className="mt-3 flex flex-col gap-2 border-t border-white/10 pt-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-[11px] leading-5 text-slate-500">Already convinced? Managed Growth keeps the existing $299/month customer-authorized Stripe path.</p><button type="button" onClick={startCheckout} disabled={Boolean(busy)} className="min-h-11 shrink-0 rounded-xl border border-emerald-300/30 bg-emerald-300/[.08] px-4 font-mono text-[9px] font-bold text-emerald-200 disabled:opacity-50">{busy==='checkout'?'OPENING STRIPE…':'START MANAGED $299/MO →'}</button></div>
      </div>
    </div>}
  </section>;
}
