      ...
      <main className="mx-auto max-w-6xl px-5 pb-24 pt-32">
        <section className="overflow-hidden rounded-[2rem] border border-cyan-400/20 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,.15),transparent_35%),linear-gradient(135deg,rgba(15,23,42,.92),rgba(0,0,0,1))] p-7 md:p-10">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-cyan-400/30 bg-cyan-400/10 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.16em] text-cyan-300"><Network size={12}/> A2A Agent Card × Trust × Governance</div>
        ...
        {result && <>
          ...
          <section className="mt-6 rounded-3xl border border-fuchsia-400/20 bg-fuchsia-400/5 p-6"><ShieldCheck className="text-fuchsia-300"/><h3 className="mt-4 font-display text-2xl font-semibold">Metadata audit first. Governance Gate before execution.</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">The trust audit tells an agent what the public card claims. For a live tool, dependency, payment, write, or other consequential action, use the public-safe governed action check exposed by the Misfit Machine Agent. The private GHOSBC runtime stays sealed behind that boundary.</p><a href="https://misfitmediahouse.com/.well-known/agent-card.json" target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.14em] text-fuchsia-300">View governed_agent_action_check <ExternalLink size={12}/></a></section>
        </>}
      </main>
      <Footer />
    </div>
