      ...
      <main className="pt-24">
        <section className="relative overflow-hidden border-b border-white/10">
          ...
          <div className="relative mx-auto max-w-7xl px-5 py-20 text-center sm:py-28">
            ...
            <h1 className="mx-auto mt-6 max-w-6xl font-display text-[clamp(3.4rem,10vw,8.2rem)] font-bold uppercase leading-[0.8] tracking-[-0.065em]">
              One house.
              <span className="block text-cyan-300">Every engine has a job.</span>
            </h1>
            <p className="mx-auto mt-7 max-w-3xl text-base leading-7 text-slate-400 sm:text-xl sm:leading-8">
              The public sees a clean set of products and outcomes. Misfit Cloud runs the estate. Misfit Governance controls risk. AI-to-AI channels distribute the work. The GHOSBC core stays sealed.
            </p>
        ...
        <section className="mx-auto max-w-7xl px-5 py-16 sm:py-24">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
            {families.map(({ id, icon: Icon, name, label, copy, href, action, tone }, index) => (
              <article id={id} key={name} className={`group flex min-h-[315px] scroll-mt-24 flex-col rounded-[1.75rem] border p-6 transition ${toneClasses[tone]} ${index < 3 ? 'xl:col-span-2' : 'xl:col-span-3'}`}>
                <div className="flex items-start justify-between gap-4">
                  <Icon size={23} />
                  <span className="rounded-full border border-white/10 px-2.5 py-1 font-mono text-[8px] uppercase tracking-[0.14em] opacity-70">{label}</span>
                </div>
                ...
                <Link to={href} className="mt-auto inline-flex items-center gap-2 pt-7 font-mono text-[10px] font-bold uppercase tracking-[0.13em]">
          ...
          <div className="mx-auto max-w-7xl px-5 py-16 sm:py-24">
            <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
              ...
              <div className="grid gap-3">
                {[
                  [Sparkles, '01 · Discover', 'Scanners, agent cards, machine catalogs, and public business surfaces create qualified demand.'],
                  [ShieldCheck, '02 · Decide', 'Misfit Governance applies privacy boundaries, risk classes, human gates, and advisory decisions.'],
                  [Workflow, '03 · Deliver', 'The correct engine performs the approved job through the existing site, API, software, or agency rail.'],
                  [LockKeyhole, '04 · Remember safely', 'Misfit Cloud keeps canonical state and evidence while protected GHOSBC material remains private.'],
                ].map(([Icon, title, copy]) => (
                  <div key={title} className="grid grid-cols-[auto_1fr] gap-4 rounded-2xl border border-white/10 bg-black/40 p-5">
                    <div className="rounded-xl border border-cyan-400/20 bg-cyan-400/10 p-2.5 text-cyan-300"><Icon size={19} /></div>
                    <div><h3 className="font-display text-xl font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{copy}</p></div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="mx-auto max-w-7xl px-5 py-16 sm:py-24">
          ...
          <div className="mt-5 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
            <h2 className="max-w-4xl font-display text-5xl font-bold uppercase leading-[0.88] tracking-[-0.04em] sm:text-7xl">One front door. Multiple ways to earn.</h2>
            <p className="max-w-md text-sm leading-7 text-slate-500">The scanner acquires. Products monetize repeatable work. The agency activates larger systems. Governance keeps higher-risk automation inside a controlled rail.</p>
          </div>
          <div className="mt-10 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {revenueRail.map(([stage, name, href, copy]) => (
              <Link key={stage} to={href} className="group flex min-h-[250px] flex-col rounded-3xl border border-white/10 bg-white/[0.025] p-6 transition hover:border-cyan-400/35">
                <div className="font-mono text-[9px] uppercase tracking-[0.16em] text-cyan-300">{stage}</div>
                <h3 className="mt-5 font-display text-2xl font-bold">{name}</h3>
                <p className="mt-4 text-sm leading-6 text-slate-500">{copy}</p>
                <div className="mt-auto inline-flex items-center gap-2 pt-6 font-mono text-[9px] uppercase tracking-[0.13em] text-slate-300">Open rail <ArrowUpRight size={12} /></div>
              </Link>
            ))}
          </div>
        </section>
