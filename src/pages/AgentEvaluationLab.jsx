...
const cardTone = {
  cyan: { label: 'text-cyan-300', hover: 'hover:border-cyan-300/30' },
  emerald: { label: 'text-emerald-300', hover: 'hover:border-emerald-300/30' },
};

function PublicLinkCard({ name, url, body, tone = 'cyan' }) {
  const style = cardTone[tone] || cardTone.cyan;
  return (
    <a href={url} className={`misfit-card-rail group rounded-2xl border border-white/10 bg-black/30 p-4 transition sm:p-5 ${style.hover}`}>
      <div className="flex items-center justify-between gap-3">
        <span className={`font-mono text-xs font-bold ${style.label}`}>{name}</span>
        <ExternalLink size={14} className={`shrink-0 text-slate-600 group-hover:${style.label}`} />
      </div>
      <p className="mt-3 text-sm leading-6 text-slate-500">{body}</p>
      <div className="mt-3 max-w-full break-all font-mono text-[10px] leading-5 text-slate-700 sm:truncate">{url}</div>
    </a>
  );
}
...
export default function AgentEvaluationLab() {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <Navbar />
      <main className="mx-auto w-full max-w-6xl overflow-x-clip px-4 pb-24 pt-28 sm:px-5 sm:pt-32">
        ...
        <section className="mt-6 grid gap-4 lg:grid-cols-[1.05fr_.95fr]">
          <div className="misfit-card-rail rounded-3xl border border-white/10 bg-white/[0.025] p-5 sm:p-6 md:p-7">
            <div className="flex items-center gap-3"><GitCompareArrows className="shrink-0 text-cyan-300"/><h2 className="font-display text-2xl font-semibold">Evaluation flow</h2></div>
            <div className="mt-5 space-y-3">{flow.map(([n, title, body]) => <div key={n} className="grid grid-cols-[34px_minmax(0,1fr)] gap-3 rounded-2xl border border-white/8 bg-black/30 p-4"><div className="flex h-8 w-8 items-center justify-center rounded-full border border-cyan-400/30 font-mono text-xs text-cyan-300">{n}</div><div className="min-w-0"><div className="font-semibold">{title}</div><div className="mt-1 text-sm leading-6 text-slate-500">{body}</div></div></div>)}</div>
          </div>

          <div className="misfit-card-rail rounded-3xl border border-white/10 bg-white/[0.025] p-5 sm:p-6 md:p-7">
            <div className="flex items-center gap-3"><BarChart3 className="shrink-0 text-fuchsia-300"/><h2 className="font-display text-2xl font-semibold">Comparative metrics</h2></div>
            <div className="mt-5 space-y-3">{metrics.map(([title, body]) => <div key={title} className="rounded-2xl border border-white/8 bg-black/30 p-4"><div className="flex items-start gap-3"><CheckCircle2 size={16} className="mt-0.5 shrink-0 text-emerald-300"/><div className="min-w-0"><div className="text-sm font-semibold">{title}</div><div className="mt-1 text-xs leading-5 text-slate-500">{body}</div></div></div></div>)}</div>
          </div>
        </section>
        ...
        <section className="misfit-card-rail mt-6 rounded-3xl border border-amber-300/20 bg-amber-300/[0.035] p-5 sm:p-6 md:p-7">
