...
import { ArrowRight, Bot, Gamepad2, Globe2, Radar, Server, Sparkles, Trophy, Users } from 'lucide-react';
...
const lanes=[
  {
    ...
    name:'GTA / FiveM',
    status:'LIVE',
    stage:'Growth OS + creator network',
    copy:'The first live Misfit Gaming OS lane: free growth audit, free founding pilot, server discovery, creator distribution and managed growth.',
    href:'/gta',
    cta:'OPEN GTA LIVE',
    live:true,
  },
  {
    ...
    name:'Minecraft',
    status:'NEXT',
    stage:'Server growth + creator distribution',
    copy:'Adapt the existing community-growth rail to server discovery, retention, Discord conversion, creator partnerships and measurable acquisition.',
  },
  {
    key:'rust',
    name:'Rust',
    status:'NEXT',
    stage:'Server population + wipe-cycle growth',
    copy:'Use the same acquisition and attribution logic around public servers, community funnels, wipe/event promotion and returning-player growth.',
  },
  {
    key:'roblox',
    name:'Roblox',
    status:'EXPANSION',
    stage:'Experience growth + creator economy',
    copy:'Focus on audience acquisition, experience conversion, retention, creator distribution and monetization analytics without pretending to control platform payout rules.',
  },
  {
    key:'fortnite',
    name:'Fortnite / UEFN',
    status:'EXPANSION',
    stage:'Island growth + build competitions',
    copy:'Creator-island growth, engagement evidence and eligible game-building competitions become a second lane alongside community-server growth.',
  },
  {
    key:'ark-dayz',
    name:'ARK / DayZ',
    status:'WATCHLIST',
    stage:'Community-server reuse lane',
    copy:'Strong architecture fit, but intentionally behind the first five so Misfit does not spread the factory across too many ecosystems before proof.',
  },
];
...
export default function GamingHub(){
  ...
  const submit=async(e)=>{
    ...
    try{
      await catalog({
        ...
        message:`MISFIT GAMING OS EXPANSION · Game: ${form.game} · ${form.message.trim()}`,
        source:'gaming_os_expansion'
      });
  ...
  return <div className="min-h-screen bg-black text-white"><Navbar/><main className="mx-auto max-w-7xl px-4 pb-24 pt-24 sm:px-5 md:pt-28">
    <section className="overflow-hidden rounded-[1.8rem] border border-cyan-400/20 bg-[radial-gradient(circle_at_12%_0%,rgba(34,211,238,.18),transparent_34%),radial-gradient(circle_at_88%_20%,rgba(217,70,239,.16),transparent_30%),linear-gradient(145deg,#061017,#05030a_62%,#000)] p-5 md:p-10">
      ...
      <p className="mt-5 max-w-4xl text-[15px] leading-7 text-slate-300">GTA/FiveM is the first proven lane. Misfit Gaming OS expands the same acquisition, attribution, creator, retention and monetization architecture into other game communities instead of rebuilding a separate product for every title.</p>
    ...
    <section className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      <Pillar icon={Radar} title="ACQUISITION" copy="Find public demand, audit the funnel and attribute where members or players came from."/>
      <Pillar icon={Users} title="CREATOR NETWORK" copy="Treat creators, streamers, clips and referrals as a distribution layer instead of random promotion."/>
      <Pillar icon={Bot} title="MISFIT AI" copy="Rank opportunities, reconsider weak loops and keep automated work inside bounded, reviewable rails."/>
      <Pillar icon={Trophy} title="COMPETITION BUILD" copy="Separately hunt game-development prize opportunities and route only eligible work into the build factory."/>
    </section>

    <section id="game-lanes" className="mt-5 rounded-3xl border border-white/10 bg-white/[.025] p-4 md:p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><div className="font-mono text-[10px] uppercase tracking-[.16em] text-fuchsia-300">GAME EXPANSION MAP</div><h2 className="mt-1 text-3xl font-black">Prove one lane. Reuse the machine.</h2></div><div className="rounded-full border border-emerald-300/20 bg-emerald-300/[.05] px-3 py-2 font-mono text-[9px] text-emerald-300">NO DUPLICATE BACKENDS</div></div>
      <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{lanes.map(lane=><GameLane key={lane.key} lane={lane}/>)}</div>
    </section>
