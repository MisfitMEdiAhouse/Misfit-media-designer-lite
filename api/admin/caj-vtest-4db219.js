...
async function vcall(path,opts={}){const t=process.env.VERCEL_OIDC_TOKEN;if(!t)return {status:503,ok:false,data:{error:'VERCEL_OIDC_TOKEN missing'}};const r=await fetch(`https://api.vercel.com${path}`,{...opts,headers:{Authorization:`Bearer ${t}`,Accept:'application/json',...(opts.body?{'Content-Type':'application/json'}:{})}});const text=await r.text();let data=null;try{data=text?JSON.parse(text):null}catch{data=text}return {status:r.status,ok:r.ok,data};}
...
export default async function handler(req,res){res.setHeader('Cache-Control','no-store');res.setHeader('X-Robots-Tag','noindex');try{
 ...
 if(action==='vercel-domain'){
   ...
   if(req.method==='POST'){
     ...
     return res.status(200).json({ok:vr.ok,status:vr.status,data:vr.data});
   }
